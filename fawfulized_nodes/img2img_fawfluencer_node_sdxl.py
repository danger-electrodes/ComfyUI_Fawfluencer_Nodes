import folder_paths
import os
from .constants import ASPECT_RATIOS, IMG2IMG_FOLDER_CUSTOM_DATAS, CONTROL_NETS, IMG2IMG_INFLUENCER_FACE_CUSTOM_DATAS, BBOX_MODELS
...
from .utilities.instant_id import apply_instantid
...
class Img2ImgFawfluencerNodeSDXL:
    ...
    def INPUT_TYPES(self):
        ...
        return {
                "required":
                {
                    ...
                    "denoise": ("FLOAT", {"default": 0.10,"min": 0.0, "max": 1.0, "step": 0.01, "precision": 2}),
    ...
    def initialize_faces(self, face_img, face_image_file):
    ...
    def generate(self, **kwargs):
        ...
        if(not is_initialized):
            self.initialize_faces(face_img, face_image_file)
        ...
        for i in range(len(imgs)):
            ...
            _, segment_anything_mask = segment_anything(self.cache_grounding_dino_model, self.cache_sam_model, img_resized, "person", 0.7)
            segment_anything_mask = image.expand_mask(segment_anything_mask, mask_grow_by, False, False, mask_blur_size, 0.0, 1.00, 1.00)
            background_mask = image.get_inverse_mask(segment_anything_mask)
            background_mask = image.set_mask_strength(background_mask, background_mask_strength)
            combined_mask = image.combine_mask(segment_anything_mask, background_mask)
            ...
            #instant id
            instant_id_model, instant_id_pos, instant_id_neg = apply_instantid(self.instant_id_model, self.insightface_model, self.instant_id_controlnet_model, best_face, model, cnet_pos, cnet_neg, 0.0, 1.0, instantid_strength, ip_strength, instantid_cnet_strength, 0.35, img_resized, None, instantid_combine_embeddings)
            
            #processing the image
            noise = sampling.get_noise(start_noise.seed + i)
            cfg_guider = sampling.get_cfg_guider(instant_id_model, instant_id_pos, instant_id_neg, cfg)
            sampler = sampling.get_sampler(sampler_name)
            sigmas = sampling.get_sigmas(instant_id_model, scheduler, steps, denoise)
            ...
            cond_zero = sampling.zero_out(pos_cond)
            details_cnet_image = process_image(details_control_net_type, upscaled_base_image)
            details_cnet_cond_pos, details_cnet_neg_cond = model_management.apply_controlnet_conditions(cond_zero, cond_zero, details_cnet_image, details_control_net_strength, details_control_net_start_at, details_control_net_end_at, vae, details_control_net_type, "sdxl")
            details_cfg_guider = sampling.get_cfg_guider(model, details_cnet_cond_pos, details_cnet_neg_cond, details_cfg)
            details_sampler = sampling.get_sampler(details_sampler_name)
            details_sigmas = sampling.get_sigmas(model, details_scheduler, details_steps, details_denoise)
            ...
            best_faces_orientation_match.append(best_face)
            images.append(final)#images.append(final)

        return (images, best_faces_orientation_match)
