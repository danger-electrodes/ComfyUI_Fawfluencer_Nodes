...
def inpaint_model_conditionning(positive, negative, pixels, vae, mask, noise_mask=True):
...
def growcontextarea_m(mask, x, y, w, h, extend_factor):
...
def combinecontextmask_m(mask, x, y, w, h, optional_context_mask):
...
def inpaint_crop_single_image(image, downscale_algorithm, upscale_algorithm, preresize, preresize_mode, preresize_min_width, preresize_min_height, preresize_max_width, preresize_max_height, extend_for_outpainting, extend_up_factor, extend_down_factor, extend_left_factor, extend_right_factor, mask_hipass_filter, mask_fill_holes, mask_expand_pixels, mask_invert, mask_blend_pixels, context_from_mask_extend_factor, output_resize_to_target_size, output_target_width, output_target_height, output_padding, mask, optional_context_mask):
    if preresize:
    ...
    _, x, y, w, h = findcontextarea_m(mask)

    if context_from_mask_extend_factor != 1:
        _, x, y, w, h = growcontextarea_m(mask, x, y, w, h, context_from_mask_extend_factor)

    _, x, y, w, h = combinecontextmask_m(mask, x, y, w, h, optional_context_mask)
...
def inpaint_crop(image, downscale_algorithm, upscale_algorithm, preresize, preresize_mode, preresize_min_width, preresize_min_height, preresize_max_width, preresize_max_height, extend_for_outpainting, extend_up_factor, extend_down_factor, extend_left_factor, extend_right_factor, mask_hipass_filter, mask_fill_holes, mask_expand_pixels, mask_invert, mask_blend_pixels, context_from_mask_extend_factor, output_resize_to_target_size, output_target_width, output_target_height, output_padding, mask=None, optional_context_mask=None):
        image = image.clone()
        if mask is not None:
            mask = mask.clone()
        ...
        for b in range(batch_size):
            ...
            outputs = inpaint_crop_single_image(
            ...
            stitcher, cropped_image, cropped_mask = outputs
...
def inpaint_stitch(stitcher, inpainted_image):
    inpainted_image = inpainted_image.clone()
    results = []
    ...
    for b in range(batch_size):
        ...
        one_image, = inpaint_stitch_single_image(one_stitcher, one_image)
...
def inpaint_stitch_single_image(stitcher, inpainted_image):
