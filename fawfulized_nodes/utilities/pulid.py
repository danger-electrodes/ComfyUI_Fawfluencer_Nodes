...
def load_pulid_model_patcher(pulid_file_path):
    model = comfy.utils.load_torch_file(pulid_file_path, safe_load=True)

    if pulid_file_path.lower().endswith(".safetensors"):
    ...
    model_patcher = PulidModel(model)

    del model
    return model_patcher


def apply_pulid(model, pulid, eva_clip, face_analysis, image, weight, start_at, end_at, method=None, noise=0.0, fidelity=None, projection=None, attn_mask=None):
