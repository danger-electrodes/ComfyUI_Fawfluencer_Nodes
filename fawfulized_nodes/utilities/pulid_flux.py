from .PulidFluxHook import pulid_forward_orig, set_model_dit_patch_replace
from .patch_util import PatchKeys, add_model_patch_option
...
FACEXLIB_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "facexlib")
...
def load_pulid_flux_model_patcher(model_path):
...
def apply_pulid_flux(model, pulid_flux, eva_clip, face_analysis, image, weight, start_at, end_at, attn_mask=None, options={}):
    model = model.clone()
