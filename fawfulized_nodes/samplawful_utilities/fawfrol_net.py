...
import os
import logging
from . import fawfulized_utils_core
...
class ControlNet(ControlBase):
    ...
    def get_control(self, x_noisy, t, cond, batched_number, transformer_options):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * self.compression_ratio != self.cond_hint.shape[2] or x_noisy.shape[3] * self.compression_ratio != self.cond_hint.shape[3]:
            ...
            self.cond_hint = fawfulized_utils_core.common_upscale(self.cond_hint_original, x_noisy.shape[3] * compression_ratio, x_noisy.shape[2] * compression_ratio, self.upscale_algorithm, "center")
            ...
            if len(self.extra_concat_orig) > 0:
                ...
                for c in self.extra_concat_orig:
                    ...
                    c = fawfulized_utils_core.common_upscale(c, self.cond_hint.shape[3], self.cond_hint.shape[2], self.upscale_algorithm, "center")
                    to_concat.append(fawfulized_utils_core.repeat_to_batch_size(c, self.cond_hint.shape[0]))
...
class ControlLora(ControlNet):
    ...
    def pre_run(self, model, percent_to_timestep_function):
        ...
        for k in sd:
            weight = sd[k]
            try:
                fawfulized_utils_core.set_attr_param(self.control_model, k, weight)
            except:
                pass

        for k in self.control_weights:
            if k not in {"lora_controlnet"}:
                fawfulized_utils_core.set_attr_param(self.control_model, k, self.control_weights[k].to(dtype).to(comfy.model_management.get_torch_device()))
    ...
    def inference_memory_requirements(self, dtype):
        return fawfulized_utils_core.calculate_parameters(self.control_weights) * comfy.model_management.dtype_size(dtype) + ControlBase.inference_memory_requirements(self, dtype)
...
def controlnet_config(sd, model_options={}):
    ...
    if unet_dtype is None:
        weight_dtype = fawfulized_utils_core.weight_dtype(sd)
...
def load_controlnet_sd35(sd, model_options={}):
    ...
    new_sd = {}
    for k in fawfulized_utils_core.MMDIT_MAP_BASIC:
        if k[1] in sd:
...
def convert_mistoline(sd):
    return fawfulized_utils_core.state_dict_prefix_replace(sd, {"single_controlnet_blocks.": "controlnet_single_blocks."})
...
def load_controlnet_state_dict(state_dict, model=None, model_options={}):
    ...
    if "controlnet_cond_embedding.conv_in.weight" in controlnet_data: #diffusers format
        ...
        diffusers_keys = fawfulized_utils_core.unet_to_diffusers(controlnet_config)
    ...
    unet_dtype = model_options.get("dtype", None)
    if unet_dtype is None:
        weight_dtype = fawfulized_utils_core.weight_dtype(controlnet_data)
...
def load_controlnet(ckpt_path, model=None, model_options={}):
    ...
    cnet = load_controlnet_state_dict(fawfulized_utils_core.load_torch_file(ckpt_path, safe_load=True), model=model, model_options=model_options)
    if cnet is None:
...
class T2IAdapter(ControlBase):
    ...
    def get_control(self, x_noisy, t, cond, batched_number, transformer_options):
        ...
        if self.cond_hint is None or x_noisy.shape[2] * self.compression_ratio != self.cond_hint.shape[2] or x_noisy.shape[3] * self.compression_ratio != self.cond_hint.shape[3]:
            ...
            self.cond_hint = fawfulized_utils_core.common_upscale(self.cond_hint_original, width, height, self.upscale_algorithm, "center").float().to(self.device)
...
def load_t2i_adapter(t2i_data, model_options={}): #TODO: model_options
    ...
    if 'adapter.body.0.resnets.0.block1.weight' in t2i_data: #diffusers format
        ...
        t2i_data = fawfulized_utils_core.state_dict_prefix_replace(t2i_data, prefix_replace)
    keys = t2i_data.keys()
